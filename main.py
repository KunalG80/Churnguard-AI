...
Run with tuning:    TUNE=1 python main.py
"""
import logging
import os
import joblib
...
# ── Optuna tuning (opt-in via env var) ────────────────────────────────────────
TUNE = os.environ.get("TUNE", "0") == "1"

if TUNE:
    from src.models.tune_xgboost import run_tuning
    logger.info("TUNE=1 — running Optuna (50 trials) …")
    active_params, study = run_tuning(X_train, y_train, cat_cols, num_cols, n_trials=50)
    logger.info("Optuna best AUC: %.4f", study.best_value)
else:
    active_params = XGB_PARAMS
    logger.info("Using config params  (set TUNE=1 to run Optuna tuning)")

# ── 5-fold stratified CV ──────────────────────────────────────────────────────
logger.info("Running %d-fold stratified CV …", CV_FOLDS)
_pos    = int((y == 1).sum()); _neg = int((y == 0).sum())
_params = {k: v for k, v in active_params.items() if k != "scale_pos_weight"}
_params["scale_pos_weight"] = _neg / _pos
...
logger.info("CV AUC: %.4f ± %.4f  (folds: %s)",
            cv_scores.mean(), cv_scores.std(), [f"{s:.3f}" for s in cv_scores])
...
# Wrap in Pipeline so schema_handler / app.py can use .named_steps
...
# ── Update threshold from PR curve ───────────────────────────────────────────
optimal_t = metrics["optimal_threshold"]
logger.info("Tip: update THRESHOLD = %.3f in src/config.py for best precision/recall balance", optimal_t)
...
fig, ax = plt.subplots()
sns.countplot(data=ml_df, x="Churn_Prediction", ax=ax)
fig.savefig(FIGURES_DIR / "churn_distribution.png"); plt.close(fig)

if "gender" in ml_df.columns:
    gr = ml_df.groupby("gender")["Churn_Prediction"].mean().reset_index()
    gr["Churn_Rate_%"] = gr["Churn_Prediction"] * 100
    fig, ax = plt.subplots()
    sns.barplot(data=gr, x="gender", y="Churn_Rate_%", ax=ax)
...
logger.info("Pipeline complete — CV AUC %.4f | Test AUC %.4f",
            cv_scores.mean(), metrics["roc_auc"])
logger.info("Reports generated in %s", REPORTS_DIR if 'REPORTS_DIR' in dir() else MODEL_PATH.parent.parent / "reports")
